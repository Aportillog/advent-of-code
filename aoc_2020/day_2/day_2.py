...
# puzzle_input = [(min, max, char, password)]

test_input = [(1, 3, 'a', 'abcde'), (1, 3, 'b', 'cdefg'), (2, 9, 'c', 'ccccccccc')]


def is_valid(entry):
    cnt = entry[3].count(entry[2])
    if int(entry[0]) <= cnt <= int(entry[1]):
        return True
    else:
        return False
...
file = open("input", 'r')
lines = [clean_string(l) for l in file.readlines()]
file.close()

count = 0
for line in lines:
    if is_valid(tuple(line.split(','))):
        count += 1

print("Correct entries: " + str(count))
...
def is_new_valid(entry):
    pos1 = int(entry[0]) - 1
    pos2 = int(entry[1]) - 1
    ch = entry[2]
    password = entry[3]

    if password[pos1] == ch and password[pos2] != ch:
        return True
    elif password[pos1] != ch and password[pos2] == ch:
        return True
    else:
        return False


assert is_new_valid(test_input[0])
assert not is_new_valid(test_input[1])
assert not is_new_valid(test_input[2])

count = 0
for line in lines:
    if is_new_valid(tuple(line.split(','))):
        count += 1

print("New correct entries: " + str(count))
