...
# None of the currency exchanges seem to have heard of them, but somehow, you'll need to find fifty of these
...
def get_puzzle_one_solution(puzzle_input):
...
test_solution_1 = 514579

res = get_puzzle_one_solution(test_list)
assert res == test_solution_1

file = open("input", 'r')
lines = [int(l) for l in file.readlines()]
file.close()

solution = get_puzzle_one_solution(lines)
...
def get_puzzle_two_solution(puzzle_input):
    for i in puzzle_input:
        for j in puzzle_input:
            for w in puzzle_input:
                if is_valid(i, j, w):
                    return i*j*w
    return 0


test_solution_2 = 241861950

res = get_puzzle_two_solution(test_list)
assert res == test_solution_2

solution = get_puzzle_two_solution(lines)
