...
# Credentials, not a passport at all! Surely, nobody would mind if you made the system temporarily
...
test_count = 0
for passport in test_input:
    if is_valid_simple(passport):
        test_count += 1

assert 2 == test_count, "Wrong valid count: {}".format(test_count)


file = open("input", 'r')
passports = [psprt.replace('\n', ' ') for psprt in file.read().split('\n\n')]
file.close()
...
print("Part one solution: " + str(valid_count))
...
def is_valid_restrictive(passport):
    """
    byr (Birth Year) - four digits; at least 1920 and at most 2002.
    iyr (Issue Year) - four digits; at least 2010 and at most 2020.
    eyr (Expiration Year) - four digits; at least 2020 and at most 2030.
    ...
    cid (Country ID) - ignored, missing or not.
    """
    if not is_valid_simple(passport):
        return False
    ...
    if not 2020 <= int(passport['eyr']) <= 2030:
        return False
    if not re.search('[0-9]+(in|cm)', passport['hgt']):
        return False
    if 'cm' in passport['hgt'] and not 150 <= int(passport['hgt'].replace('cm', '')) <= 193:
        return False
    if 'in' in passport['hgt'] and not 59 <= int(passport['hgt'].replace('in', '')) <= 76:
        return False
...
txt_valid_input = """pid:087499704 hgt:74in ecl:grn iyr:2012 eyr:2030 byr:1980
hcl:#623a2f
...
iyr:2010 hgt:158cm hcl:#b6652a ecl:blu byr:1944 eyr:2021 pid:093154719"""
test_valid_input = [parse_passport(entry.replace('\n', ' ')) for entry in txt_valid_input.split('\n\n')]

txt_invalid_input = """eyr:1972 cid:100
hcl:#18171d ecl:amb hgt:170 pid:186cm iyr:2018 byr:1926
...
eyr:2038 hcl:74454a iyr:2023
pid:3556412378 byr:2007"""
test_invalid_input = [parse_passport(entry.replace('\n', ' ')) for entry in txt_invalid_input.split('\n\n')]

test_count = 0
for passport in test_valid_input:
    if is_valid_restrictive(passport):
        test_count += 1

assert len(test_valid_input) == test_count, "Wrong valid count: {}".format(test_count)

test_count = 0
for passport in test_invalid_input:
    if not is_valid_restrictive(passport):
        test_count += 1

assert len(test_invalid_input) == test_count, "Wrong valid count: {}".format(test_count)

file = open("input", 'r')
passports = [parse_passport(psprt.replace('\n', ' ')) for psprt in file.read().split('\n\n')]
file.close()
...
print("Part two solution: " + str(valid_count))
