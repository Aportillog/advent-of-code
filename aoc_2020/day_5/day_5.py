...
# in. Start with the whole list of rows; the first letter indicates whether the seat is in the front (0 through 63)
...
# seats on the plane (numbered 0 through 7). The same process as above proceeds again, this time with only
...
def decode_seat(boarding_pass):
    ...
    col = boarding_pass[7:].replace('R', '1').replace('L', '0')
    return int(row, 2), int(col, 2)


def get_seat_id(boarding_pass):
    row, col = decode_seat(boarding_pass)
    return row * 8 + col
...
seats = [get_seat_id(bp) for bp in boarding_passes]

print("Part one solution: " + str(max(seats)))
...
while i > 0:
    i -= 1
    if i not in seats:
        if i + 1 not in seats or i - 1 not in seats:
            continue
        else:
            break

print("Part two solution: " + str(i))
